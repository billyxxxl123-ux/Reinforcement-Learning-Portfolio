...
if torch is not None:
    ...
    class MultiAssetDDPG:
        ...
        def _utility_wealth(self, W):
            ...
            if self.a == 0:
                return W
            return -self.reward_scale * np.exp(-self.a * W) / self.a
        ...
        def _update_networks(self, update_actor=True):
        ...
        def solve(self):
            ...
            sched_c = optim.lr_scheduler.ReduceLROnPlateau(
                self.critic_optimizer, mode='min', factor=self.lr_plateau_factor,
                patience=self.lr_plateau_patience, min_lr=self.lr_plateau_min_lr)
            sched_a = optim.lr_scheduler.ReduceLROnPlateau(
                self.actor_optimizer, mode='min', factor=self.lr_plateau_factor,
                patience=self.lr_plateau_patience, min_lr=self.lr_plateau_min_lr)
            best_return = -np.inf
            best_actor_state = self._actor_state_cpu_clone(self.actor)

            use_loss_es = (self.early_stop_k is not None and self.early_stop_epsilon is not None and self.early_stop_k > 0)
            prev_ep_loss = None
            loss_stable = 0
            last_mean_c = None
            last_mean_a = None
            ...
            for episode in range(self.num_episodes):
                ...
                for update_step in range(self.updates_per_episode):
                    update_actor_now = (update_step % 2 == 0)
                    out = self._update_networks(update_actor=update_actor_now)
                    if out is not None:
                        c_loss, a_loss = out
                        sum_c += c_loss
                ...
                # 评估与记录（注意缩进，与 for update_step 同级）
                if episode % self.eval_interval == 0:
                    avg_return = self._evaluate_policy()
                    self.history['episodes'].append(episode)
                    self.history['eval_utility'].append(avg_return)
                    if last_mean_c is not None:
                        self.history['critic_loss'].append(last_mean_c)
                        self.history['actor_loss'].append(last_mean_a)
                    else:
                        self.history['critic_loss'].append(np.nan)
                        self.history['actor_loss'].append(np.nan)

                    lr_a = self.actor_optimizer.param_groups[0]["lr"]
                    lr_c = self.critic_optimizer.param_groups[0]["lr"]
                    print(f"Episode {episode}, utility: {avg_return:.4f}, lr_actor: {lr_a:.2e}, lr_critic: {lr_c:.2e}")
                    if last_mean_c is not None:
                        print(f"  loss: critic={last_mean_c:.6f}, actor={last_mean_a:.6f}")
                    else:
                    ...
                    if len(self.replay_buffer) >= self.batch_size:
                        with torch.no_grad():
                            sample_states, _, _, _, _ = self.replay_buffer.sample(min(self.batch_size, 100), terminal_boost=0.0)
                            sample_states = torch.FloatTensor(sample_states[:100])
                            sample_actions = self.actor(sample_states)
                            q_vals = self.critic(sample_states, sample_actions)
                            print(f"  Q mean: {q_vals.mean():.6f}, std: {q_vals.std():.6f}, min: {q_vals.min():.6f}, max: {q_vals.max():.6f}")
...
# -------------------------------------------------------------
# 6. Plot DDPG training loss & utility
# -------------------------------------------------------------
def plot_ddpg_training(ddpg_solver, figsize=(12, 4), save_path=None):
    """
    Plot critic loss, actor loss, and evaluation utility recorded during DDPG training.

    Parameters
    ----------
    ddpg_solver : MultiAssetDDPG instance
        The solver after training (must have the 'history' attribute).
    figsize : tuple, optional
        Figure size.
    save_path : str or Path, optional
        If provided, save the figure to this path.
    """
    if not hasattr(ddpg_solver, 'history') or not ddpg_solver.history['episodes']:
        print("No training history recorded. Make sure DDPG has been trained.")
        return

    hist = ddpg_solver.history
    episodes = hist['episodes']
    critic_loss = hist['critic_loss']
    actor_loss = hist['actor_loss']
    eval_utility = hist['eval_utility']

    fig, axes = plt.subplots(1, 2, figsize=figsize)

    # Left plot: losses
    ax1 = axes[0]
    ax1.plot(episodes, critic_loss, 'o-', color='C0', label='Critic Loss (MSE)', alpha=0.7)
    ax1.plot(episodes, actor_loss, 's-', color='C1', label='Actor Loss (-Q)', alpha=0.7)
    ax1.set_xlabel('Episode')
    ax1.set_ylabel('Loss')
    ax1.set_title('DDPG Training Losses')
    ax1.legend()
    ax1.grid(True, alpha=0.3)

    # Right plot: evaluation utility
    ax2 = axes[1]
    ax2.plot(episodes, eval_utility, 'o-', color='green', label='Avg Utility (CARA)')
    ax2.set_xlabel('Episode')
    ax2.set_ylabel('Average Utility')
    ax2.set_title('Evaluation Utility during Training')
    ax2.legend()
...
if __name__ == "__main__":
    ...
    # Plot DDPG training losses if DDPG was run
    if res['ddpg_solver'] is not None:
        plot_ddpg_training(res['ddpg_solver'])

if __name__ == "__main__":
    # Large Return Large Risk Example
    r = 0.05
    mu = np.array([0.07, 0.10, 0.20])
    var = [0.1, 0.2, 0.3]
    cov = covariance_matrix(var)
    p0 = np.array([0.3, 0.2, 0.4, 0.1])
    W0 = 1.0
...
if __name__ == "__main__":
    # Large Return Small Risk Example
    r = 0.05
    mu = np.array([0.07, 0.10, 0.20])
    var = [0.03, 0.05, 0.10]
    cov = covariance_matrix(var)
...
if __name__ == "__main__":
    # Small Return Large Risk Example
    r = 0.05
...
if __name__ == "__main__":
    # Small Return Small Risk Example
    r = 0.05
    mu = np.array([0.01, 0.02, 0.05])
    var = [0.03, 0.05, 0.10]
    cov = covariance_matrix(var)
